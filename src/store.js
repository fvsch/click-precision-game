...
function getPlaygroundStartSize() {
  const { MIN, MAX, STEP, DEFAULT } = PLAYGROUND_SIZE;
  if (typeof window === "object" && window.innerWidth) {
    const extra = Math.floor(window.innerWidth - 40) - MIN;
    if (!isNaN(extra) && extra > 0) {
      const validExtra = extra - (extra % STEP);
      return clamp(MIN, MIN + validExtra, 600);
    }
  }
  return DEFAULT;
}
